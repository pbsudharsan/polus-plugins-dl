...
import cv2
from scipy.ndimage import find_objects, gaussian_filter, generate_binary_structure, label, binary_fill_holes
...
def distance_to_boundary(masks):
    ...
    Args:
    masks(array[int]):  2D or 3D array.size [Ly x Lx] or [Lz x Ly x Lx], 0=NO masks; 1,2,...=mask labels

    Returns:
    dist_to_bound(array): 2D or 3D array.size [Ly x Lx] or [Lz x Ly x Lx]
...
def masks_to_edges(masks, threshold=1.0):
    ...
    Returns:
    edges(array): 2D or 3D .size [Ly x Lx] or [Lz x Ly x Lx], True pixels are edge pixels
...
def masks_to_outlines(masks):
    ...
    Returns:
    outlines(array): 2D or 3D array.size [Ly x Lx] or [Lz x Ly x Lx], True pixels are outlines
