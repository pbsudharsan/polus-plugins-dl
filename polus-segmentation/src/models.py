...
class Cellpose():
    ...
    def eval(self, x, batch_size=8, channels=None, invert=False, normalize=True, diameter=30., do_3D=False,
             anisotropy=None,
             net_avg=True, augment=False, tile=True, tile_overlap=0.1, resample=False, flow_threshold=0.4,
        ...
        if diameter is not None and diameter != 0:
            ...
            if self.pretrained_size is not None and rescale is None and not do_3D:
                tic = time.time()
                diams, _ = self.sz.eval(x, channels=channels, invert=invert, batch_size=batch_size, augment=augment,
                                        tile=tile)
                rescale = self.diam_mean / diams
...
class UnetModel():
    ...
    def _train_net(self, train_data, train_labels,
        ...
        print('>>>> LR: %0.5f, batch_size: %d, weight_decay: %0.5f' % (
        self.learning_rate, self.batch_size, self.weight_decay))
        print('>>>> ntrain = %d' % nimg)
        if test_data is not None:
